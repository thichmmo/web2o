...
import { adminLogsAPI } from '../../services/adminApi';
...
const ActivityLogs = () => {
  ...
  const loadLogs = async () => {
    setLoading(true);
    try {
      const response = await adminLogsAPI.getLogs({ ...filters, page: currentPage, limit: 50 });
    ...
    <div className="px-4 py-6 sm:px-0">
      ...
      {pagination.totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between bg-white p-4 rounded-lg shadow">
          ...
          <button disabled={currentPage === pagination.totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="px-4 py-2 border rounded disabled:opacity-50">Next</button>
        </div>
      )}
    </div>
  );
